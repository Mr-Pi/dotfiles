...
function install_x11 {
	...
	local sys=`md5sum /usr/share/terminfo/x/xterm-screen-256color | awk '{print $1}'`
	local org=`md5sum "${BASE_DIR}/xterm-screen-256color" | awk '{print $1}'`
	[ "$sys" != "$org" ] && sudo cp -vi "${BASE_DIR}/xterm-screen-256color" "/usr/share/terminfo/x/xterm-screen-256color"
}
